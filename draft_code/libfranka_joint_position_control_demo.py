...
def send2robot(conn, q):
    # max_scale = 1.0
    q = np.asarray(q)
    # scale = np.linalg.norm(q)
    # if scale > max_scale:
    #     q = [q[i] * max_scale / scale for i in range(8)]
    # qdot += np.asarray([1e-5]*7)
    send_msg = np.array2string(q, precision=5, separator=',', suppress_small=True)[1:-1]
    send_msg = "s," + send_msg + ","
...
def main():
    ...
    q_control = [0.40061, -0.78517, -0.00026, -2.35582, 0.00066, 1.57028, 0.78522, 2.0]

    for idx in range(7):
        print("Attempting to read state")
        ...
        send2robot(conn, q_control)
        print("I sent that joint position command to the robot")
